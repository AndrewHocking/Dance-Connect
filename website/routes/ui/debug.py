...
def create_demo_opportunities():
    ...
    posts = [
        {
            ...
            "closing_date": datetime.now(timezone.utc) + timedelta(days=random.randint(15, 130)),

            "tags": getRandVals(dance_tags),
            "location_type": LocationType.IN_PERSON,
            "location": random.choice(locations),
            "start_date": datetime.now(timezone.utc) + timedelta(weeks=random.randint(5, 15)),
            "end_date": datetime.now(timezone.utc) + timedelta(weeks=random.randint(17, 80)),
            "is_paid": True,
            "pay":  '2 cents',
    ...
    for post in posts:
        create_opportunity_post(
            **post, poster_id=random.choice(users).id, display_description=post["description"])
